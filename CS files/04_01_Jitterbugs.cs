...
#region Members
  ...
  public class JitterBug
  {
    ...
    // static field - a static field is shared by all instances of the same class
    // if one instance updates its value, all istances will see the updated value
    static int nBugs = 0;
    ...
    public JitterBug()
    {
      id = nBugs; // the newly created bug will have the current count as id
      rnd = new Random(id);
      pos = new Point3d(rnd.NextDouble() - 0.5, rnd.NextDouble() - 0.5, 0);
      vel = new Vector3d(rnd.NextDouble() - 0.5, rnd.NextDouble() - 0.5, 0) * 0.25;
      nBugs++; // every time I create a new JitterBug, the total count increases
    }
